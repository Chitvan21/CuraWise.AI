...
def get_predicted_values(patient_symptoms):
    ...
    # The model expects 132 features as shown in the Training.csv
    expected_features = 132
    input_vector = np.zeros(expected_features)
    for symptom in patient_symptoms:
        if symptom in symptoms_dict:
            # Only set values for symptoms that exist in the dictionary
            symptom_index = symptoms_dict[symptom]
            # Only set the value if the index is within the expected range
...
# ========= WORKFLOW TAB =========
    ...
    with col2:
        st.image(r"CuraWise-WorkFlow-Tab.png")
...
# ========= Recommendations TAB =========
    ...
    with col2:
        st.image(r"CuraWise-Recommendations.png")
...
# ========= Report Generation TAB =========
    ...
    with col1:
        if st.session_state.get("signedOut", False):
            ...
            st.markdown("* Then go to the Login Page and Login Yourself.")
    with col2:
        st.image(r"CuraWise-Generate-Report.png")
